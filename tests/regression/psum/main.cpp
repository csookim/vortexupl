...
// blackbox.sh --driver=rtlsim --app=sort --perf=1
...
void gen_ref_data(uint32_t num_points, uint32_t num_thread) {
  // answer
  ref_data.resize(num_thread);

  int stride = num_points / num_thread;

  for (uint32_t i = 0; i < num_thread; ++i) {
    int sum = 0;
    for (uint32_t j = 0; j < stride; ++j) {
      int idx = stride * i + j;
      TYPE ref_value = src_data.at(idx);
      sum += ref_value;
    }
    ref_data.at(i) = sum;
  }
...
int run_test(const kernel_arg_t& kernel_arg,
  ...
  std::cout << "verify result" << std::endl;  
  {
    ...
    // int ref = ref_data.at(0);
    // int cur = buf_ptr[0];

    // if (cur != ref) {
    //   std::cout << "FAILED! " << ref << " " << cur << std::endl;
    //   return 1;  
    // }
    int errors = 0;
    // auto buf_ptr = (TYPE*)staging_buf.data();
    for (uint32_t i = 0; i < kernel_arg.num_tasks; ++i) {
      TYPE ref = ref_data.at(i); // answer
      TYPE cur = buf_ptr[i]; // result
      // std::cout << std::dec << ref << " " << cur << std::endl;
      if (cur != ref) {
...
int main(int argc, char *argv[]) {  
  ...
  gen_ref_data(num_points, num_tasks);
